...
def generate_launch_description():
    # Find the package containing the launch file
    pkg_project_bringup = FindPackageShare('project_bringup').find('project_bringup')
    
    # Define the path to the included launch file
    device_launch = os.path.join(pkg_project_bringup, 'launch', 'device.launch.py')
    
    # Include the device launch file
    include_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(device_launch)
    )
