...
- Added the ability to download storage file prior to uploading a new once
- Improved warning for uploading storage file and deleting current data
- Can delete base stations from the map
- Security and General updates done to supporting packages
- Bing Maps removed as an option for a map type
